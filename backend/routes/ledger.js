...
router.post('/', protect, async (req, res) => {
    const { date, clientName, description, amount, type, ownerId } = req.body;
    const newEntry = {
        id: `led-${uuidv4()}`,
        date,
        clientName,
    ...
    try {
        await db.query(
            'INSERT INTO ledger_entries (id, date, clientName, description, amount, type, ownerId) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [newEntry.id, newEntry.date, newEntry.clientName, newEntry.description, newEntry.amount, newEntry.type, newEntry.ownerId]
        );
