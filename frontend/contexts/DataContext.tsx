...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const duplicateClient = async (client: Client) => {
        // Remove `id` and `avatar` — these will be generated by the backend
        const { id, avatar, ...clientData } = client;

        // Ensure type matches Omit<Client, 'id' | 'avatar'>
        const newClient = await api.createClient(clientData as Omit<Client, "id" | "avatar">);
        setClients(prev => [newClient, ...prev]);
        return newClient;
    };
