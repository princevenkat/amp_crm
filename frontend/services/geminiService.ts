...
if (!process.env.API_KEY) {
  console.warn(
    "API_KEY environment variable not set. GenAI features will be disabled."
  );
}
...
export const summarizeClient = async (client: Client): Promise<string> => {
  if (!process.env.API_KEY)
    return "GenAI is not configured. Please set the API_KEY environment variable.";
  try {
    const prompt = `
      ...
      ${
        client.applicationType === "Joint"
          ? `Second Applicant: ${client.applicants[1].firstName} ${client.applicants[1].surname}`
          : ""
      }
      ...
      ${
        client.productDetails?.businessWritten
          ? `Business Written: ${client.productDetails.businessWritten}`
          : ""
      }
      ${
        client.productDetails?.mortgage
          ? `Mortgage Amount: $${client.productDetails.mortgage.mortgageLoanAmount.toLocaleString()} with ${
              client.productDetails.mortgage.lender
            }`
          : ""
      }
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              ...
              description: {
                type: Type.STRING,
                description: "A short, helpful description of the task.",
              },
              dueDate: {
                type: Type.STRING,
                description: "Suggested due date (YYYY-MM-DD).",
              },
            },
